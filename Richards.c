...
double DivisionRate(double F, int X1, int K, double beta)
{
	return F*(1-pow((double)X1/K, beta))*X1;
}
...
void TotalTransitionRate(double FA, double GA, int XA, int K, double *array, double beta)
{
	array[0] = DivisionRate(FA, XA, K, beta);
...
int main()
{
	...
	const double FA = 1.0, GA = 0, beta = 2.0;
	const int K = 100000, XA0 = 100, Nit = 10000;
	double t = 0, Tvect[2], tlist[301], deltat = .1, tmax = 30;
	...
	for (k = 0; k < Nit; k++)
	{
		...
		{
			TotalTransitionRate(FA, GA, XA, K, Tvect, beta);
			CumSum(Tvect, 1);
